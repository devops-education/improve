...
public class SecurityConfig {
	...
	public SecurityFilterChain configure(HttpSecurity http, MvcRequestMatcher.Builder mvc) {
		try {
			...
			http = http.authorizeHttpRequests((authorize) -> authorize.requestMatchers(mvc.pattern("/"))
				.permitAll()
				.requestMatchers(new AntPathRequestMatcher("/login.faces"))
				.permitAll()
				.requestMatchers(new AntPathRequestMatcher("/jakarta.faces.resource/**"))
				.permitAll()
				.requestMatchers(new AntPathRequestMatcher("/user.faces"))
			...
			http = http.logout((logout) -> {
				logout.logoutSuccessUrl("/login.faces").deleteCookies("JSESSIONID");
			});
	 ...
	 * @return InMemoryUserDetailsManager - a manager that keeps all the users' info in
	 * the memory
	 */
	@Bean
