...
public abstract class AbstractFeedbackSummaryCrudMBean<T extends IdBean<Long>> extends CrudMBean<T, Long> {
	...
	protected List<ChallengeFeedback> sortChallengeFeedback(List<ChallengeFeedback> list) {
		Comparator<ChallengeFeedback> comparator = (ChallengeFeedback o1,
				ChallengeFeedback o2) -> o2.getChallenge().getChallengeInterviews().size()
	...
	protected List<RecommendationFeedback> sortRecommendationFeedback(List<RecommendationFeedback> list) {
		Comparator<RecommendationFeedback> comparator = (RecommendationFeedback o1,
				RecommendationFeedback o2) -> o2.getRecommendation().getRecommendationInterviews().size()
	...
	protected List<RecommendationFeedback> recommendationFeedbackList(Collection<Recommendation> recommendations,
		...
		Map<Recommendation, RecommendationFeedback> recommendationRecommendationFeedbackMap = recommendationFeedbackList
			.stream()
		...
		for (Recommendation bean : recommendations) {
			...
			if (cf == null) {
				cf = RecommendationFeedback.builder().recommendation(bean).user(userMBean.getLoggedUser()).build();
			}
			result.add(cf);
		}
