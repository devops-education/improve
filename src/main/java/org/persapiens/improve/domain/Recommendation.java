...
public class Recommendation implements IdBean<Long>, Comparable<Recommendation> {
	...
	public List<Link> getLinkSortedByChallengeChallengeInterviewsSizeList() {
		...
		Collections.sort(result, Comparator.comparing(Link::getChallenge, (s1, s2) -> {
			return Long.valueOf(s1.getChallengeInterviews().size() - s2.getChallengeInterviews().size()).intValue();
		}).reversed());
		return result;
	}
	...
	public Set<Recommendation> getRecommendationConflicts() {
		...
		result.addAll(recommendation1Conflicts.stream()
			.map(RecommendationsConflict::getRecommendation2)
			.collect(Collectors.toSet()));
		result.addAll(recommendation2Conflicts.stream()
