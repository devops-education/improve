...
public class Link implements IdBean<LinkId>, Comparable<Link> {
	...
	@JoinColumn(name = "challenge", nullable = false, insertable = false, updatable = false,
			foreignKey = @ForeignKey(name = "fk_link_challenge"))
	private Challenge challenge;

	@Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
	@ManyToOne
	@JoinColumn(name = "recommendation", nullable = false, insertable = false, updatable = false,
			foreignKey = @ForeignKey(name = "fk_link_recommendation"))
	...
	public int compareTo(Link o) {
		return Comparator.comparing(Link::getChallenge).thenComparing(Link::getRecommendation).compare(this, o);
	}
